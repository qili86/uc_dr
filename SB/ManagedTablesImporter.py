...
class CommandExecutor:
    ...
    def _execute_and_collect(self, _iterable: Iterable) -> list[any]:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            futures = {executor.submit(self._spark_command_execute, x[0]): x for x in _iterable}
...
class ManagedTablesImporter:
    """
        A class to import the managed tables.
    """
    def __init__(self, spark, meta_storage_location, metastore_type, import_type='dr'):
        self.spark = spark
    ...
    def __get_new_catalogs_and_schemas(self, managed_tables_primary: DataFrame) -> DataFrame:
        ...
        managed_tables_secondary = self.spark.read.format("delta").table("system.information_schema.tables")
        
        new_catalogs_schemas = (managed_tables_primary.alias("p").join(managed_tables_secondary.alias("s"), (managed_tables_primary.table_catalog == managed_tables_secondary.table_catalog) & (managed_tables_primary.table_schema == managed_tables_secondary.table_schema), "inner").select("p.table_catalog", "p.table_schema").withColumn("create_catalog_stmt_template", lit(create_catalog_stmt_template)).withColumn("create_schema_stmt_template", lit(create_schema_stmt_template)).withColumn("create_catalog_stmt", replace(col("create_catalog_stmt_template"), lit("<catalog_name>"), col("table_catalog"))).withColumn("create_schema_stmt", replace(replace(col("create_schema_stmt_template"), lit("<catalog_name>"), col("table_catalog")), lit("<schema_name>"), col("table_schema"))))
        return new_catalogs_schemas.select("create_catalog_stmt", "create_schema_stmt")
    
    def __create_catalogs_and_schemas(self, managed_tables: DataFrame):
        new_catalogs_and_schemas = self.__get_new_catalogs_and_schemas(managed_tables).collect()
        catalog_stmts = []
        schema_stmts = []
    ...
    def __grant_permissions(self):
        primary = self.spark.read.format("delta").load(self.managed_tables_privileges_storage_path)
    ...
    def import_managed_tables(self):
        managed_tables = self.spark.read.format("delta").load(self.managed_tables_storage_path)
        # create catalogs and schema if it needed to be created first. 
        self.__create_catalogs_and_schemas(managed_tables)
        # deep clone managed tables.
        self.__clone_managed_tables(managed_tables)
        # grant permissions
        if self.metastore_type.lower() == 'uc': 
            self.__grant_permissions()

# COMMAND ----------

importer= ManagedTablesImporter(spark, meta_storage_location, metastore_type)
importer.import_managed_tables()
